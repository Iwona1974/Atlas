const PASS={entry:"1974",sound:"brązowe",image:"espresso",taste:"oddech",smell:"palce"};
const STORAGE="atlasDoznanProgressV15";const COMPLETE_STORAGE="atlasDoznanCompleteV15";let unlocked=0,atlasComplete=false;
try{unlocked=Math.max(0,Math.min(4,Number(sessionStorage.getItem(STORAGE)||0)));atlasComplete=sessionStorage.getItem(COMPLETE_STORAGE)==="1"}catch(e){unlocked=0;atlasComplete=false}let currentScreen=null,currentSound=null,currentImage=null,currentTaste=null,currentSmellSection=null,currentSmellPlace=null,galleryTimer=null,galleryIndex=0,navigationTimer=null;
const $=id=>document.getElementById(id);const screens={};document.querySelectorAll(".screen").forEach(s=>screens[s.id]=s);
const WELCOME=["Są doświadczenia, które trwają zaledwie moment. A mimo to zostają z nami na długo.","To mgnienie oka. Zasłyszany dźwięk. Smak pozostający na języku. Zapach, do którego chce się wracać. Delikatne muśnięcie skóry.","Nieuchwytne. Nieoczywiste. A jednak tak intensywne, że potrafią zostać w pamięci na zawsze.","Niektóre wywołują emocje. Inne przywołują wspomnienia. Jeszcze inne po prostu warto poczuć.","Nie da się ich zatrzymać. Można jedynie do nich wracać.","Każdy zmysł otwiera inne drzwi. Reszta zależy od Ciebie."];
const SOUND_INTRO=["Jest niewidzialny, a mimo to potrafi zostawić po sobie ślad.","Uspokaja, dodaje odwagi, wzrusza, rozbawia, przywołuje wspomnienia. Czasem mówi więcej niż słowa.","Muzyka jest chyba najbardziej osobistym z języków. Nie trzeba jej tłumaczyć.","Dwie osoby mogą słuchać tego samego utworu, a usłyszeć zupełnie różne historie."];
const IMAGE_INTRO=["Jedni dostrzegają światło. Inni kolory, detale.","Ale najważniejsze są emocje ukryte między spojrzeniami.","Sposób, w jaki patrzymy, często mówi o nas więcej niż to, na co patrzymy.","Być może właśnie dlatego każdy widzi świat trochę inaczej."];
...
function showScreen(id,delay=800){clearTimeout(navigationTimer);if(currentScreen)currentScreen.classList.replace("screen-visible","screen-hidden");clearSecretInputs();navigationTimer=setTimeout(()=>{const n=screens[id];if(!n)return;n.classList.replace("screen-hidden","screen-visible");n.scrollTop=0;currentScreen=n;clearSecretInputs()},delay)}
function go(id,delay=800,record=true){if(record){try{history.pushState({atlas:true,screen:id},"",location.href)}catch(e){}}showScreen(id,delay)}
function typed(el,a,done,d=65,p=220){el.innerHTML="";let t=0;a.forEach(x=>{const pe=document.createElement("p");x.split(" ").forEach((w,i,arr)=>{const s=document.createElement("span");s.className="typed-word";s.textContent=w+(i<arr.length-1?" ":"");pe.appendChild(s);setTimeout(()=>s.classList.add("visible"),t);t+=d});el.appendChild(pe);t+=p});setTimeout(done,t+300)}
...
function checkKey(input,msg,pass,next){const field=$(input);if(field.value.trim().toLowerCase()!==pass){$(msg).textContent="To jeszcze nie ten klucz.";$(msg).classList.add("message-visible");field.value="";field.focus();return}field.value="";$(msg).textContent="";$(msg).classList.remove("message-visible");saveUnlock(next);go("worlds-screen")}
function startGalleryTimer(list,firstDelay=6500){clearTimeout(galleryTimer);galleryTimer=setTimeout(function advance(){showGallery((galleryIndex+1)%list.length);galleryTimer=setTimeout(advance,6500)},firstDelay)}function openGallery(list,returnId){clearTimeout(galleryTimer);const st=$("gallery-stage"),dots=$("gallery-dots");st.innerHTML="";dots.innerHTML="";galleryIndex=0;list.forEach((src,i)=>{const im=document.createElement("img");im.className="gallery-image"+(i===0?" active":"");im.src=src;im.alt=`Zdjęcie ${i+1} z ${list.length}`;st.appendChild(im);const d=document.createElement("button");d.className="gallery-dot"+(i===0?" active":"");d.type="button";d.setAttribute("aria-label",`Pokaż zdjęcie ${i+1} z ${list.length}`);d.onclick=()=>{showGallery(i);startGalleryTimer(list,10000)};dots.appendChild(d)});$("close-gallery").dataset.return=returnId;go("gallery-screen");startGalleryTimer(list)}function showGallery(i){galleryIndex=i;document.querySelectorAll(".gallery-image").forEach((x,j)=>x.classList.toggle("active",j===i));document.querySelectorAll(".gallery-dot").forEach((x,j)=>x.classList.toggle("active",j===i))}
function recipeHTML(t){return `<img src="${t.img}" alt="${t.dish}"><h2>${t.dish}</h2><h3>Składniki:</h3><ul>${t.ingredients.map(x=>`<li>${x}</li>`).join("")}</ul><h3>Przygotowanie:</h3><ol>${t.steps.map(x=>`<li>${x}</li>`).join("")}</ol><div class="recipe-tip"><h3>Mała podpowiedź…</h3><p>${t.tip}</p></div>`}
function openTouch(){go("touch-screen");$("return-to-atlas").classList.remove("visible-control");setTimeout(()=>{typed($("touch-text"),TOUCH,()=>setTimeout(()=>$("return-to-atlas").classList.add("visible-control"),1200),78,420);const paragraphs=$("touch-text").querySelectorAll("p");if(paragraphs.length)paragraphs[paragraphs.length-1].classList.add("touch-closing")},1000)}
function formatAudioTime(seconds){
 const value=Number.isFinite(seconds)?Math.max(0,Math.floor(seconds)):0;
 const minutes=Math.floor(value/60);const secs=String(value%60).padStart(2,"0");return `${minutes}:${secs}`
}
function stopSpotify(){
 const audio=$("atlas-audio");
 if(audio){audio.pause();try{audio.currentTime=0}catch(e){}audio.removeAttribute("src");audio.load()}
...
function setPanelState(panelId,open,instant=false){
 ...
 clearTimeout(panel._fadeTimer);clearTimeout(panel._heightTimer);
 panel.dataset.animating="1";
 if(instant){
  panel.style.transition="none";
  panel.classList.toggle("collapsed",!open);
  panel.classList.toggle("panel-fading",!open);
  panel.style.maxHeight=open?panel.scrollHeight+"px":"0px";
  panel.style.marginTop=open?"20px":"0px";
  panel.setAttribute("aria-hidden",open?"false":"true");
  requestAnimationFrame(()=>{panel.style.transition="";panel.dataset.animating="0"});
  return;
 }
 if(open){
  ...
  panel.style.maxHeight="0px";panel.style.marginTop="0px";
  void panel.offsetHeight;
  panel.style.maxHeight=panel.scrollHeight+"px";panel.style.marginTop="20px";
  panel._fadeTimer=setTimeout(()=>panel.classList.remove("panel-fading"),260);
  panel._heightTimer=setTimeout(()=>{panel.style.maxHeight=panel.scrollHeight+"px";panel.dataset.animating="0"},1050);
 }else{
  panel.style.maxHeight=panel.scrollHeight+"px";panel.style.marginTop="20px";
  ...
  panel._fadeTimer=setTimeout(()=>{
   panel.style.maxHeight="0px";panel.style.marginTop="0px";
   panel._heightTimer=setTimeout(()=>{panel.classList.add("collapsed");panel.dataset.animating="0"},980);
  },820);
 }
}
function togglePanel(panelId){const panel=$(panelId);if(!panel||panel.dataset.animating==="1")return;setPanelState(panelId,panel.classList.contains("collapsed"))}
// init
currentScreen=screens["login-screen"];renderWorlds();
try{history.replaceState({atlas:true,screen:"login-screen"},"",location.href)}catch(e){}
const SCREEN_PARENT={
 "welcome-screen":"login-screen","title-screen":"welcome-screen","worlds-screen":"title-screen",
 "sound-intro-screen":"worlds-screen","sound-tiles-screen":"sound-intro-screen","sound-detail-screen":"sound-tiles-screen","sound-ending-screen":"sound-tiles-screen",
 "image-intro-screen":"worlds-screen","image-tiles-screen":"image-intro-screen","image-detail-screen":"image-tiles-screen","image-ending-screen":"image-tiles-screen",
 "taste-intro-screen":"worlds-screen","taste-tiles-screen":"taste-intro-screen","taste-detail-screen":"taste-tiles-screen","taste-ending-screen":"taste-tiles-screen",
 "smell-intro-screen":"worlds-screen","smell-tiles-screen":"smell-intro-screen","smell-section-screen":"smell-tiles-screen","smell-place-screen":"smell-section-screen","smell-ending-screen":"smell-tiles-screen",
 "touch-screen":"worlds-screen","video-screen":"image-detail-screen"
};
...
// Uzupełnienia finalne: czytelne komunikaty, obsługa klawiatury i bezpieczne zamykanie multimediów.
["entry-password","sound-key-input","image-key-input","taste-key-input","smell-key-input"].forEach(id=>{
 const input=$(id); if(!input)return;
 input.addEventListener("input",()=>{
  const message=input.parentElement.querySelector(".message");
  if(message){message.textContent="";message.classList.remove("message-visible")}
 });
});
function closeVideo(){
 const video=$("image-video");
 video.pause(); video.currentTime=0; video.removeAttribute("src"); video.load();
 go("image-detail-screen",0);
}
$("close-video").onclick=closeVideo;
$("image-video").addEventListener("play",()=>stopSpotify());
document.addEventListener("keydown",e=>{
 if(e.key!=="Escape")return;
 if(currentScreen===screens["gallery-screen"]){clearTimeout(galleryTimer);go($("close-gallery").dataset.return,0)}
